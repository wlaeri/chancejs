...
describe("::Chance ::Text ::#paragraph", () => {
  let chance: Chance;
  beforeEach(() => {
    chance = new Chance();
  });
  ...
  describe("When executes paragraph with sentences argument", () => {
    it("returns the correct result", () => {
      times(100, () => {
        let p = chance.paragraph({ sentences: 5 });
        expect(typeof p).toBe("string");
  ...
  describe("When executes paragraph with linebreak argument", () => {
    it("returns the correct result", () => {
      times(100, () => {
        let rand = chance.natural({ min: 1, max: 50 });
        let p = chance.paragraph({ sentences: rand, linebreak: true });
        expect(typeof p).toBe("string");
