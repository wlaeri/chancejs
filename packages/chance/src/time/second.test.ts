...
describe("::Chance ::Time ::#second", () => {
  ...
  describe("When executes second() with min and max arguments", () => {
    it("returns correct result", () => {
      times(1000, () => {
        let s = chance.second({ min: 18, max: 35 });
        expect(typeof s).toBe("number");
  ...
  describe("When pass min arg with value greater than max arg", () => {
    it("throws error", () => {
      const fn = () => chance.second({ min: 10, max: 5 });
