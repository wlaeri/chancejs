...
describe("::Chance :: Helpers :: #shuffle", () => {
  let chance: Chance;
  beforeEach(() => {
    chance = new Chance();
  });
  ...
  it("returns an array of the same size", () => {
    ...
    times(1000, () => {
      const arrCopy = [...arr];
      const shuffled = chance.shuffle({ array: arrCopy });
  ...
  it("returns a well shuffled array", () => {
    ...
    times(10000, () => {
      chance.shuffle({ array: arrayCopy }).map((item, index) => {
  ...
  it("it does not destroy original array", () => {
    ...
    times(1, () => {
      const arrayCopy = [...arr];
      const shuffled = chance.shuffle({ array: arrayCopy });
      expect(shuffled.length).toEqual(5);
