...
// plop-imports
import { TimestampGenerator, TimestampOptions } from "@chancejs/timestamp";
...
export class Chance implements IChance {
  ...
  constructor(options?: ChanceOptions) {
    ...
    // plop-constructor
    this.timestampGenerator = new TimestampGenerator({ seed, generator });
  ...
  timestamp(options?: TimestampOptions): number {
    return this.timestampGenerator.timestamp(options);
  }
