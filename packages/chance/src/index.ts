...
import { PadGenerator, PadOptions } from "@chancejs/pad";
...
export class Chance implements IChance {
  ...
  private padGenerator: PadGenerator;
  ...
  constructor(options?: ChanceOptions) {
    ...
    this.padGenerator = new PadGenerator({ seed, generator });
  }
  ...
  pad(options: PadOptions): string {
    return this.padGenerator.pad(options);
  }
}
